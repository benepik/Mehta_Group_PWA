        ...
        _createClass(ChangeAddressPage, [{
          ...
          value: function radioGroupChange(ev, obj) {
            ...
            if (obj.is_primary == '1') {
              ...
              this.zone.run(function () {
                ...
                _this2.apiService.apiCallWithLoginToken(src_assets_constant__WEBPACK_IMPORTED_MODULE_4__.URLS.SkipUrl, apiKey).subscribe(function (result) {
          ...
          value: function chagngeAddress() {
            ...
            this.zone.run(function () {
              var apiKey = {
                "address_fetch_type": "change",
                "redeem_for": _this3.redeem_for.request_page,
                "request_page": "dealer",
                "request_for": _this3.redeem_for.customer_id,
                "request_user_type": _this3.redeem_for.employee_type
              };

              _this3.apiService.apiCallWithLoginToken(src_assets_constant__WEBPACK_IMPORTED_MODULE_4__.URLS.AddressUrl, apiKey).subscribe(function (result) {
                console.log("AddressUrl: ", result);

                _this3.apiService.presentLoadingClose();

                _this3.addres = result;

                if (result.success == 1) {
                  for (var i = 0; i < _this3.addres.data.data.length; i++) {
                    if (_this3.addres.data.data[i].is_primary == '1' || _this3.addres.data.data[i].is_primary == 1) {
                      _this3.imgArr = _this3.addres.data.data[i].auto_id;
                      console.log("this.imgArr: ", _this3.imgArr);
                    } else {
                      // this.imgArr= '';
                      console.log("this.imgArr: ", _this3.imgArr);
                    }
