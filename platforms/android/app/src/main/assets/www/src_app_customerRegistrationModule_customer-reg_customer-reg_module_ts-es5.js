    ...
    11701: function _(__unused_webpack_module, __webpack_exports__, __webpack_require__) {
      ...
      var _CustomerRegPage = /*#__PURE__*/function () {
        function CustomerRegPage(route, platform, navController, zone, apiService, sendData) {
          ...
          this.searchString = '';
          this.isItemAvailable = false; // items = [];
        ...
        _createClass(CustomerRegPage, [{
          ...
          value: function customerReg() {
            ...
            this.zone.run(function () {
              ...
              _this.apiService.apiCallWithLoginToken(src_assets_constant__WEBPACK_IMPORTED_MODULE_4__.URLS.CustReg, apiKey).subscribe(function (result) {
                console.log("CustReg: ", result); // this.apiService.presentLoadingClose();
                ...
                _this.apiService.showToastMessage(JSON.stringify(err), 'top', 3000, 'redBg'); // this.apiService.presentLoadingClose();
          ...
          value: function showItemList(id) {
            ...
            this.zone.run(function () {
              ...
              _this2.apiService.apiCallWithLoginToken(src_assets_constant__WEBPACK_IMPORTED_MODULE_4__.URLS.CustRegDataUrl, apiKey).subscribe(function (result) {
                ...
                if (result.success == 1) {
                  if (_this2.custmReg == undefined) {
                    _this2.custmReg = result.data;
                  } else {
                    _this2.custmReg = _this2.custmReg.concat(result.data);
                  }

                  _this2.showImg = "";
                  _this2.errorMessage = '';
                  console.log("custmReg : ", _this2.custmReg);
                } else {
                  if (_this2.value == 0) {
          ...
          value: function loadData(event) {
            ...
            setTimeout(function () {
              _this3.value = _this3.custmReg.length;

              _this3.showItemList("rejected");
            }, 500);
          }
        }, {
          key: "loadDataActive",
          value: function loadDataActive(event) {
            var _this4 = this;
            ...
            setTimeout(function () {
              _this4.value = _this4.custmReg.length; // for (let i = 0; i < this.buttonData.length; i++) {

              if (_this4.buttonData[0].tab_status == 'selected_tab') {
                _this4.showItemList("active");
              } else {
                _this4.showItemList("block");
              }
            }, 500);
          }
        }, {
          key: "loadDataPending",
          value: function loadDataPending(event) {
            var _this5 = this;

            this.infiniteScroll_call = true;
            this.infiniteScroll_val = event;
            setTimeout(function () {
              _this5.value = _this5.custmReg.length;

              _this5.showItemList("pending");
            }, 500);
          }
        }, {
          ...
          value: function changedTabs(item) {
            ...
            this.custmReg = [];
            this.value = 0; // this.showImg={};

            this.zone.run(function () {
              if (item.tab_status != "selected_tab") {
                item.tab_status = "selected_tab";

                if (_this6.pageTitle == "Approved List") {
                  if (item.tab_name == "Active Customers") {
                    item.tab_status = 'selected_tab';

                    _this6.showItemList("active");
                  } else {
                    item.tab_status = 'unselected_tab';

                    _this6.showItemList("block");
                  }
                }

                for (var i = 0; i < _this6.buttonData.length; i++) {
                  if (item.tab_name != _this6.buttonData[i].tab_name) {
                    _this6.buttonData[i].tab_status = "unselected_tab";
                  } else {
                    _this6.buttonData[i].tab_status = "selected_tab";
                  }
                }
              }
            });
          ...
          value: function changedTab(item) {
            ...
            this.zone.run(function () {
              _this7.custmReg = [];
              _this7.value = 0; // this.showImg={};

              if (item.tab_status != "selected_tab") {
                item.tab_status = "selected_tab";
                _this7.pageTitle = item.tab_name;

                if (_this7.pageTitle == "Approved List") {
                  _this7.buttonData[0].tab_status = "selected_tab";
                  _this7.buttonData[1].tab_status = "unselected_tab";

                  _this7.showItemList("active");
                } else if (_this7.pageTitle == "Rejected List") {
                  _this7.showItemList("rejected");
                } else {
                  _this7.showItemList("pending");
                }

                if (item.show_no_data == "1") {
                  _this7.showImg = item.no_data_image;
                } else {}

                for (var i = 0; i < _this7.buttData.length; i++) {
                  if (item.tab_name != _this7.buttData[i].tab_name) {
                    _this7.buttData[i].tab_status = "unselected_tab";
                  } else {
                    _this7.buttData[i].tab_status = "selected_tab";
                  }
                }

                console.log("this.buttData :", _this7.buttData);
              } else {
                if (item.show_no_data == "1") {
                  _this7.showImg = item.no_data_image;
